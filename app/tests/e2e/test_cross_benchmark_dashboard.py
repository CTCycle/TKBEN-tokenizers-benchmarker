    ...
    page.route(
        ...
        lambda route: route.fulfill(json={
            ...
            "dashboard": {"available_widget_ids": ["efficiency.speed", "vocabulary.size"], "available_metric_keys": ["efficiency.speed", "vocabulary.size"], "unavailable_selected_metric_keys": [], "widgets": [
                {"widget_id": "efficiency.speed", "metric_keys": ["efficiency.speed"], "category_key": "efficiency", "category_label": "Efficiency", "label": "Tokenization speed", "description": "Mean throughput", "unit": "tokens/s", "display_format": "number", "default_visualization": "interval_bar", "compatible_visualizations": ["interval_bar", "dot_whisker"], "default_visible": True, "width": "standard", "points": [{"tokenizer": "alpha", "value": 1500, "interval_low": 1400, "interval_high": 1600}, {"tokenizer": "beta", "value": 1200, "interval_low": 1100, "interval_high": 1300}], "distributions": [], "buckets": [], "histogram_bins": []},
                {"widget_id": "vocabulary.size", "metric_keys": ["vocabulary.size"], "category_key": "vocabulary", "category_label": "Vocabulary", "label": "Vocabulary size", "description": "Vocabulary entries", "unit": "tokens", "display_format": "number", "default_visualization": "bar", "compatible_visualizations": ["bar", "horizontal_bar"], "default_visible": True, "width": "standard", "points": [{"tokenizer": "alpha", "value": 5000, "interval_low": None, "interval_high": None}, {"tokenizer": "beta", "value": 6000, "interval_low": None, "interval_high": None}], "distributions": [], "buckets": [], "histogram_bins": []},
                {"widget_id": "latency.distribution", "metric_keys": ["latency.distribution"], "category_key": "latency", "category_label": "Latency", "label": "Encode latency distribution", "description": "Per-document latency", "unit": "ms", "display_format": "milliseconds", "default_visualization": "box_plot", "compatible_visualizations": ["box_plot", "histogram"], "default_visible": True, "width": "wide", "points": [], "distributions": [{"tokenizer": "alpha", "min": 1, "q1": 2, "median": 3, "q3": 4, "max": 6, "sample_count": 10}, {"tokenizer": "beta", "min": 2, "q1": 3, "median": 4, "q3": 5, "max": 7, "sample_count": 10}], "buckets": [], "histogram_bins": [{"tokenizer": "alpha", "bin_low": 1, "bin_high": 4, "count": 8, "proportion": 0.8}, {"tokenizer": "alpha", "bin_low": 4, "bin_high": 7, "count": 2, "proportion": 0.2}, {"tokenizer": "beta", "bin_low": 1, "bin_high": 4, "count": 7, "proportion": 0.7}, {"tokenizer": "beta", "bin_low": 4, "bin_high": 7, "count": 3, "proportion": 0.3}]},
                {"widget_id": "fragmentation.bucket", "metric_keys": ["fragmentation.bucket"], "category_key": "fragmentation", "category_label": "Fragmentation", "label": "Word-length bucket comparison", "description": "Pieces per word by bucket", "unit": "pieces/word", "display_format": "ratio", "default_visualization": "grouped_bar", "compatible_visualizations": ["grouped_bar"], "default_visible": True, "width": "wide", "points": [], "distributions": [], "buckets": [{"tokenizer": "alpha", "bucket": "short_1_4", "value": 1.2}, {"tokenizer": "alpha", "bucket": "long_9_plus", "value": 1.8}, {"tokenizer": "beta", "bucket": "short_1_4", "value": 1.1}, {"tokenizer": "beta", "bucket": "long_9_plus", "value": 2.0}], "histogram_bins": []}
            ]},
        }),
    )
...
def test_cross_benchmark_dashboard_customization_and_accessible_data(
    ...
    expect(page.get_by_role("button", name="Use Vertical bar chart for Vocabulary size")).to_have_attribute("aria-pressed", "true")
    ...
    expect(page.locator(".benchmark-visualization-button").first).to_have_css("width", "32px")
    expect(page.locator(".benchmark-visualization-button").first).to_have_css("height", "32px")
    ...
    def assert_centered_content(widget_label: str, maximum_width: float) -> None:
        ...
        assert abs((stage_box["x"] + stage_box["width"] / 2) - (content_box["x"] + content_box["width"] / 2)) <= 1
    ...
    bar_rectangles = page.locator('article[aria-label="Vocabulary size widget"] .recharts-bar-rectangle')
    expect(bar_rectangles).to_have_count(2)
    bar_widths = bar_rectangles.evaluate_all("elements => elements.map(element => element.getBoundingClientRect().width)")
    assert max(bar_widths) <= 45

    page.get_by_role("button", name="Use Horizontal bar chart for Vocabulary size").click()
    expect(page.get_by_role("button", name="Use Horizontal bar chart for Vocabulary size")).to_have_attribute("aria-pressed", "true")
    page.reload()
    expect(page.get_by_role("button", name="Use Horizontal bar chart for Vocabulary size")).to_have_attribute("aria-pressed", "true")
