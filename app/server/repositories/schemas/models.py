...
class Dataset(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(16), nullable=False, default="loading", server_default="loading"
    )
    ...
    benchmark_reports: Mapped[list[BenchmarkReport]] = relationship(
        back_populates="dataset",
        cascade="all, delete-orphan",
        passive_deletes=True,
        lazy="raise",
...
class DatasetDocument(Base):
    ...
    dataset_id: Mapped[int] = mapped_column(
        ForeignKey("dataset.id", ondelete="CASCADE"), nullable=False
    )
...
class AnalysisSession(Base):
    ...
    parameters: Mapped[dict[str, Any]] = mapped_column(
        JSONObject(), nullable=False, default=dict, server_default="{}"
    )
    selected_metric_keys: Mapped[list[str]] = mapped_column(
    ...
    __table_args__ = (
        ...
        Index(
            "ix_analysis_session_dataset_created_id", "dataset_id", "created_at", "id"
        ),
    )
    dataset: Mapped[Dataset] = relationship(
        back_populates="analysis_sessions", lazy="raise"
    )
    metric_values: Mapped[list[MetricValue]] = relationship(
        ...
        passive_deletes=True,
        lazy="raise",
        overlaps="metric_values,document",
    )
    histograms: Mapped[list[HistogramArtifact]] = relationship(
        back_populates="session",
        cascade="all, delete-orphan",
        passive_deletes=True,
        lazy="raise",
...
class MetricValue(Base):
    ...
    json_value: Mapped[dict[str, Any] | list[Any] | None] = mapped_column(
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "(CASE WHEN numeric_value IS NOT NULL THEN 1 ELSE 0 END + CASE WHEN text_value IS NOT NULL THEN 1 ELSE 0 END + CASE WHEN json_value IS NOT NULL THEN 1 ELSE 0 END) = 1",
            name="ck_metric_exactly_one_value",
        ),
        CheckConstraint(
            "length(trim(metric_key)) > 0", name="ck_metric_value_key_nonblank"
        ),
        Index(
            ...
            "metric_key",
            unique=True,
            sqlite_where=and_(document_id.is_(None)),
            postgresql_where=and_(document_id.is_(None)),
        ),
        Index(
            ...
            "document_id",
            unique=True,
            sqlite_where=and_(document_id.is_not(None)),
            postgresql_where=and_(document_id.is_not(None)),
        ),
    ...
    session: Mapped[AnalysisSession] = relationship(
        back_populates="metric_values", lazy="raise", overlaps="metric_values,document"
    )
    document: Mapped[DatasetDocument | None] = relationship(
...
class HistogramArtifact(Base):
    ...
    session_id: Mapped[int] = mapped_column(
        ForeignKey("analysis_session.id", ondelete="CASCADE"), nullable=False
    )
    metric_key: Mapped[str] = mapped_column(String(255), nullable=False)
    ...
    __table_args__ = (
        UniqueConstraint(
            "session_id", "metric_key", name="uq_histogram_artifact_session_metric_key"
        ),
        CheckConstraint(
            "length(trim(metric_key)) > 0", name="ck_histogram_metric_key_nonblank"
        ),
        CheckConstraint("min_value <= max_value", name="ck_histogram_range"),
        Index("ix_histogram_artifact_session_id", "session_id", "id"),
    )
    session: Mapped[AnalysisSession] = relationship(
...
class Tokenizer(Base):
    ...
    source: Mapped[str] = mapped_column(
        String(32), nullable=False, default="huggingface", server_default="huggingface"
    )
    created_at: Mapped[datetime] = mapped_column(UTCDateTime(), nullable=False)
    __table_args__ = (
        CheckConstraint("length(trim(name)) > 0", name="ck_tokenizer_name_nonblank"),
        CheckConstraint(
            "source IN ('huggingface', 'custom')", name="ck_tokenizer_source"
        ),
    )
    reports: Mapped[list[TokenizerReport]] = relationship(
        back_populates="tokenizer",
        cascade="all, delete-orphan",
        passive_deletes=True,
        lazy="raise",
    )
    vocabularies: Mapped[list[TokenizerVocabulary]] = relationship(
        back_populates="tokenizer",
        cascade="all, delete-orphan",
        passive_deletes=True,
        lazy="raise",
...
class TokenizerVocabulary(Base):
    ...
    tokenizer_id: Mapped[int] = mapped_column(
        ForeignKey("tokenizer.id", ondelete="CASCADE"), nullable=False
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint("token_id >= 0", name="ck_tokenizer_token_id"),
    )
...
class TokenizerReport(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    tokenizer_id: Mapped[int] = mapped_column(
        ForeignKey("tokenizer.id", ondelete="CASCADE"), nullable=False, unique=True
    )
    report_version: Mapped[int] = mapped_column(Integer, nullable=False)
    created_at: Mapped[datetime] = mapped_column(UTCDateTime(), nullable=False)
    metadata_json: Mapped[dict[str, Any]] = mapped_column(
        "metadata", JSONObject(), nullable=False
    )
    token_length_histogram: Mapped[dict[str, Any]] = mapped_column(
...
class BenchmarkReport(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("schema_version > 0", name="ck_benchmark_schema_version"),
        CheckConstraint(
            "documents_processed >= 0 AND tokenizers_count >= 0",
            name="ck_benchmark_counts",
        ),
        Index(
            "ix_benchmark_report_dataset_created_id", "dataset_id", "created_at", "id"
        ),
        Index("ix_benchmark_report_created_id", "created_at", "id"),
    )
    dataset: Mapped[Dataset] = relationship(
...
class HFAccessKey(Base):
    ...
    __table_args__ = (
        Index(
            ...
            "is_active",
            unique=True,
            sqlite_where=and_(is_active.is_(True)),
