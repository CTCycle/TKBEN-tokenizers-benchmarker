...
export class CrossBenchmarkPageComponent {
  ...
  protected openRun(): void {
    ...
    this.runForm.patchValue({
      ...
      runName: '',
      maxDocuments: this.store.report()?.config.max_documents ?? benchmarkDefaults?.default_max_documents ?? 1000,
      warmupTrials: benchmarkDefaults?.default_warmup_trials ?? 2,
      timedTrials: benchmarkDefaults?.default_timed_trials ?? 8,
