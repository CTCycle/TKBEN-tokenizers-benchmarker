...
PARTIAL; provider and PostgreSQL gates remain BLOCKED. See the [Dataset and
Settings QA record](../../assets/QA/tkben-t2-01-t4-02-t5-02-dataset-settings-validation-20260924/README.md).
...
commit `e870e135b5163044bf11565ff4ead8e3a72a4871`; both frontend and backend jobs succeeded. T5-06 remains PARTIAL
because release publication evidence is separate. Hugging Face and PostgreSQL
gates remain BLOCKED as listed below. T5-05 is OUT_OF_SCOPE for the Windows x64
release target.
...
- Recent local evidence covers report management, dashboard customization,
  ...
  responsive matrix. Remaining validation debt includes physical low-disk
  recovery, conditional provider/database routes, hosted release publication,
  and manual PDF inspection of other visualization overrides and larger
  vocabularies.
...
| persistence.postgresql-runtime | BLOCKED | External PostgreSQL initialization, migration locking, concurrency, and runtime equivalence with SQLite. | Unit coverage exercises initializer branches. A 2026-09-24 probe found a local PostgreSQL server responding at 127.0.0.1:5433, but repository settings select embedded SQLite and provide no PostgreSQL endpoint or credentials; the Docker engine is unavailable, so no disposable target was confirmed. | SQLite success must not be promoted to PostgreSQL equivalence, and the detected host service is not presumed disposable. | Requires an approved disposable PostgreSQL target and configured connection/credentials. | — | None | [persistence](architecture/persistence.md), [configuration](runtime/configuration.md) | Provision a disposable target and run the documented PostgreSQL integration validation. |
...
| integration.huggingface-discovery-and-download | BLOCKED | Live Hugging Face tokenizer discovery, metadata filtering, gated access, repository download, and provider-backed tokenizer report flow. | V-20260924: T4-02 downloaded one anonymous public Wikitext dataset and persisted it across restart. This does not exercise tokenizer discovery or provider-backed reports; the two Hugging Face discovery tests remained skipped. | No current live claim is made for tokenizer repository compatibility, gated access, credential handling, or provider-backed reporting. | Requires an approved provider key/network and explicit opt-in live tests. | — | None | [backend API](architecture/backend_api.md), [testing and quality](coding/testing_and_quality.md), [tokenizer E2E](../../app/tests/e2e/test_tokenizers_api.py), [Tokenizers QA record](../../assets/QA/tkben-t2-04-t5-03-tokenizer-validation-20260924/README.md), [Dataset and Settings QA record](../../assets/QA/tkben-t2-01-t4-02-t5-02-dataset-settings-validation-20260924/README.md) | Run tokenizer-provider discovery and report-flow tests with an approved credential and record pass/skip/failure separately. |
...
| benchmark.dashboard-and-pdf-export | WORKING | Normalized report-v5 widgets, visualizations, data tables, dashboard layout, and PDF export contracts. | V-20260923: populated dashboard rendered at four viewport sizes; baseline deltas, tables, customization, ordering, and reload persistence passed. T3-05 exercised the live dataset, tokenizer, and benchmark export route with 2/2/4-page PDFs; all pages were visually inspected, including a horizontal-bar override and the matching 23-token data table. The label and axis-layout fixes are covered by focused unit tests. See the [T3-05 QA record](../../assets/QA/tkben-t3-05-dashboard-pdf-exports-20260923.md). | Manual PDF inspection covers the seven visible benchmark widgets and one horizontal-bar override. Other visualization overrides and vocabulary exports beyond 23 entries remain uninspected. | — | 2026-09-23 | unit + E2E + manual | [benchmark dashboard](ui/benchmark_dashboard.md), [T3-03/04 QA record](../../assets/QA/tkben-t3-03-04-cross-benchmark-workflows-20260923.md), [T3-05 QA record](../../assets/QA/tkben-t3-05-dashboard-pdf-exports-20260923.md), [export tests](../../app/tests/unit/test_dashboard_export_service.py), [export route tests](../../app/tests/unit/server/api/test_exports_routes.py) | Extend manual PDF inspection to non-default visualization overrides and large vocabulary exports before promoting the full component scope. |
...
| ui.settings-page | WORKING | Settings tabs, typed controls, inline validation, persistence, conflict handling, reset, and Keys section navigation. | V-20260922: the rendered Settings route had no overlay or console errors; Chrome browser E2E passed 2/2 with all 16 controls, boundary errors, cross-field recovery, persistence, reload hydration, conflict, reset, and new-operation effects. V-20260924: Settings Data and empty Keys states rendered without document-level overflow at four viewport sizes; ArrowRight/End route-tab navigation passed. | The rendered key-management lifecycle remains outside the current slice; T5-02 only inspected the empty Keys state. | — | 2026-09-24 | unit + E2E + manual | [configuration](runtime/configuration.md), [experience](ui/experience.md), [Settings E2E](../../app/tests/e2e/test_settings_ui.py), [T1-02 closure QA record](../../assets/QA/tkben-t1-02-settings-boundary-20260922.md), [Tier 1 QA record](../../assets/QA/tkben-tier1-20260921.md), [Dataset and Settings QA record](../../assets/QA/tkben-t2-01-t4-02-t5-02-dataset-settings-validation-20260924/README.md) | Complete the separate rendered key-management lifecycle before promoting the full page scope. |
...
| test-infrastructure.hosted-ci-and-release-evidence | PARTIAL | Current hosted CI result and committed detailed QA/release evidence. | Hosted CI run 36060275436 passed for storage-validation commit `e870e135b5163044bf11565ff4ead8e3a72a4871`; both frontend and backend jobs succeeded. Earlier validation commit CI run 36006192379 also passed. CI runner OS is automation infrastructure, not a supported application platform. | Release publication evidence was not checked and cannot be inferred from CI. | Release audit is outside this validation scope. | 2026-09-24 | hosted CI | [CI workflow](../../.github/workflows/ci.yml), [run 36060275436](https://github.com/CTCycle/TKBEN-tokenizers-benchmarker/actions/runs/36060275436), [earlier run 36006192379](https://github.com/CTCycle/TKBEN-tokenizers-benchmarker/actions/runs/36006192379), [release procedure](runtime/release.md), [T3-03/04 QA record](../../assets/QA/tkben-t3-03-04-cross-benchmark-workflows-20260923.md) | Validate release publication and correlate it with a separately selected release SHA. |
...
| --- | --- | --- | --- |
| benchmark.dashboard-and-pdf-export | WORKING | T3-05 passed for populated dataset, tokenizer, and benchmark reports, including one visualization override and chart/data-table parity. Remaining gap: manual inspection of other visualization overrides and large vocabulary exports before promoting the broader component scope. | MEDIUM |
| data.large-file-and-disk-exhaustion | PARTIAL | V-20260924 accepted the exact 25 MiB API upload limit, rejected one byte over, persisted a 24 MiB CSV, and triggered real SQLite `SQLITE_FULL` after a committed batch for both upload and dataset-backed imports. Partial rows were removed and same-name retries passed after restoring the SQLite page limit. Actual host filesystem exhaustion remains untested. See the [dataset storage QA record](../../assets/QA/tkben-data-upload-storage-20260924/README.md). | MEDIUM |
...
| persistence.postgresql-runtime | BLOCKED | Run disposable PostgreSQL migration, concurrency, rollback, and runtime-equivalence checks. A local server answered on 127.0.0.1:5433, but no disposable database target or credentials are configured; repository settings still select SQLite. | MEDIUM |
| test-infrastructure.hosted-ci-and-release-evidence | PARTIAL | Hosted CI run 36060275436 passed for storage-validation commit `e870e135b5163044bf11565ff4ead8e3a72a4871`; committed QA evidence is present. Release publication remains unverified. | MEDIUM |
...
| T3-05 | yes | yes | PASS | V-20260923: official Windows launcher and isolated reports exercised all three populated exports. The route returned 200 `application/pdf`, exact attachment names, and 2/2/4 page counts; `pdfinfo` and rendered output agreed, and all eight pages were visually inspected. The dataset source report had 77 selected metric keys, with its three-document statistics visible; tokenizer vocabulary 23 and seven benchmark widgets with the saved horizontal-bar override and value-23 data table were also confirmed. Fixes address clipped long tokenizer labels and visualization-specific axis titles. Export contract/route/backend tests passed 17/17; frontend malformed-histogram normalization passed 6/6, lint and production build passed; one pytest `cache_dir` warning remains. See the [T3-05 QA record](../../assets/QA/tkben-t3-05-dashboard-pdf-exports-20260923.md). |
...
| T4-04 | yes | no | BLOCKED | A local PostgreSQL server responds at 127.0.0.1:5433, but repository settings select embedded SQLite and no PostgreSQL endpoint or credentials are configured. The host service is not presumed disposable. |
...
| T5-06 | yes | yes | PARTIAL | Hosted CI run 36060275436 completed successfully for storage-validation commit `e870e135b5163044bf11565ff4ead8e3a72a4871`; release publication evidence remains unverified. See the [T3-03/04 QA record](../../assets/QA/tkben-t3-03-04-cross-benchmark-workflows-20260923.md) and [Tokenizers QA record](../../assets/QA/tkben-t2-04-t5-03-tokenizer-validation-20260924/README.md). |
