...
remain BLOCKED; T4-02, T5-02, and the remaining responsive matrix are still
open as listed below. T5-05 is OUT_OF_SCOPE for the Windows x64 release target.
...
| integration.huggingface-discovery-and-download | BLOCKED | Live Hugging Face discovery, gated/public access, metadata filtering, download, and provider-backed tokenizer report flow. | V-20260924: the local custom-tokenizer report flow passed under T2-05. The two Hugging Face discovery cases remained skipped; no provider-backed download or report was attempted. | No current live claim is made for provider availability, credential handling, or remote repository compatibility. | Requires a usable provider key/network and explicit opt-in live tests. | — | None | [backend API](architecture/backend_api.md), [testing and quality](coding/testing_and_quality.md), [tokenizer E2E](../../app/tests/e2e/test_tokenizers_api.py), [Tokenizers QA record](../../assets/QA/tkben-t2-04-t5-03-tokenizer-validation-20260924/README.md) | Run with an approved provider credential and record pass/skip/failure separately. |
...
| Component | Status | Scope | Evidence | Known Issues | Blocker | Last Validated | Validation Level | Related Docs | Next Action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ui.route-shell-and-empty-states | VALIDATED | Primary navigation, route loading, dataset/tokenizer empty states, benchmark empty state, and Settings shell. | V-20260920: four routes rendered in the in-app browser; browser console error log was empty. V-20260924: Tokenizers empty, loading, controlled-error, and populated states rendered without horizontal overflow at four viewport sizes. See the [Tokenizers QA record](../../assets/QA/tkben-t2-04-t5-03-tokenizer-validation-20260924/README.md). | Dataset and Settings route visuals have not been reviewed across the full responsive state matrix; this remains tracked under `ui.responsive-visual-matrix`. | — | 2026-09-24 | E2E + manual | [experience](ui/experience.md), [components and patterns](ui/components_and_patterns.md), [app-flow E2E](../../app/tests/e2e/test_app_flow.py), [Tokenizers QA record](../../assets/QA/tkben-t2-04-t5-03-tokenizer-validation-20260924/README.md) | Validate the Dataset and Settings state matrices under T5-02. |
