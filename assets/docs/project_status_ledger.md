...
- Windows launcher execution, runtime-settings persistence, populated report
  dashboards, tokenizer reports, PDF export, hosted CI, and the documented
...
| runtime.startup.local-webapp | VALIDATED | FastAPI readiness, Angular production preview, local API proxy, and empty-state route loading. | V-20260920: backend health returned 200, frontend returned 200, and the browser rendered Dataset, Tokenizers, Cross Benchmark, and Settings routes with no console errors. | This validates direct local service startup; it does not validate every launcher menu branch. | — | 2026-09-20 | E2E + manual | [startup](runtime/startup.md), [runtime modes](runtime/modes.md), [system overview](architecture/system_overview.md), [startup test](../../app/tests/unit/server/test_app_startup.py) | Repeat through the canonical Windows launcher after launcher changes. |
| runtime.windows-launcher | WORKING | start_on_windows.ps1 dependency bootstrap, source-stale build detection, readiness checks, maintenance menu, and process cleanup. | Launcher source, runtime documentation, and the production build are present; the direct backend/preview path succeeded in V-20260920. | The current bootstrap did not invoke -Launch, redirected diagnostics, or -KillAll through the launcher itself. | — | — | None | [startup](runtime/startup.md), [deployment](runtime/deployment.md), [release procedure](runtime/release.md) | Run start_on_windows.ps1 -Launch and -KillAll in the next release validation. |
...
| test-infrastructure.local-quality-gates | VALIDATED | Backend compile, Ruff, BasedPyright, SQLite initialization, unit tests, OpenAPI smoke, frontend lint, unit tests, and production build. | V-20260920: compile passed for tracked backend source; Ruff passed with a non-failing cache permission warning; BasedPyright reported 0 errors and 1,914 warnings; 323 backend tests, 12 frontend test files/55 tests, lint, build, and OpenAPI smoke passed. | Hosted CI and the full browser E2E matrix are separate gates. | — | 2026-09-20 | unit + integration | [testing and quality](coding/testing_and_quality.md), [CI workflow](../../.github/workflows/ci.yml), [test runner](../../app/tests/run_tests.bat) | Keep hosted and live-provider gates explicit in future reports. |
