...
| --- | --- | --- | --- |
| runtime.windows-launcher | PARTIAL | Force denied-termination behavior in a disposable privilege-isolated Windows harness; the repeatable harness is now present, while clean bootstrap, every menu route, stamp repair, redirect conflicts, invalid ports, and port reacquisition remain covered by V-20260922. | HIGH |
...
| T0-02 | yes | yes | PARTIAL | Clean bootstrap, warm reuse, missing/malformed stamps, stale build, backend-only repair, invalid ports, redirected conflict, and a real port reacquisition race passed; the launcher contract suite now passes 19/19 and a safety-gated real-denial harness is present. Permission-denied termination was not safely forced in this host boundary, so promotion remains pending isolated evidence. |
