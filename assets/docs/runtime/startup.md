...
- loads `settings/.env` before backend configuration and database modules are imported, then validates the required structured settings from `settings/configurations.json`; invalid booleans, missing fields, and unknown JSON blocks fail fast
- optionally shows backend logs in a dedicated terminal for interactive sessions when `BACKEND_LOGS_VISIBLE=true` (the default when absent; the only accepted values are `true` and `false`); redirected `-Launch` runs capture backend stdout and stderr under `app/resources/logs` so health-check failures include actionable diagnostics
