...
their own space so the switches remain visible without overlapping the title.
Dataset and tokenizer histogram cards use the same title-row treatment for
their `Histogram` and `CDF` choices. Dataset word-cloud frames expand to fill
the available card content area, and the layout keeps each displayed word
inside the frame.
