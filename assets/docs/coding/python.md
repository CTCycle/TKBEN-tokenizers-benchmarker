...
- Keep dependency declarations aligned with `app/server/pyproject.toml`.
- Treat `app/server/uv.lock`, when generated by `uv sync`, as local install metadata rather than a tracked source file.
