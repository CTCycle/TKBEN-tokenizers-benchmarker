...
- The launcher uses the canonical backend environment at `app/server/.venv` and local dependency metadata generated by `uv sync`, builds the frontend when configured, starts both services, and opens the configured UI URL.
